...
##### 📌 스택(Stack)
- **구조**: 선입후출(LIFO, Last In First Out) 방식의 메모리 영역
- **용도**: 함수 호출 정보(매개변수, 지역 변수, 반환 주소 등) 저장
- **특징**:
  - 크기가 제한적이고 고정됨
  - 함수가 끝나면 자동으로 해제됨
  - 접근 속도가 매우 빠름
- **장점**: 관리가 단순하고 성능이 좋음
- **단점**: 큰 데이터를 저장하기 어렵고, 수명이 짧음
...
- **특징**:
  - 크기가 크고 유연함
  - 동적 할당/해제가 필요 → 가비지 컬렉션(GC)으로 관리
  - 접근 속도가 상대적으로 느림
- **장점**: 다양한 크기의 데이터를 유연하게 저장 가능
- **단점**: 관리 비용이 크고, 잘못된 참조 관리 시 메모리 누수 발생 가능
